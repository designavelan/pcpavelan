...
def renderizar(df_nuvem, df_codigos, filtros_selecionados):
    ...
    # ==========================================
    # MAPEAMENTO DE CÓDIGOS DE PAUSA (INTERVALOS)
    # ==========================================
    codigos_pausa = []
    if not df_codigos.empty and 'tipo' in df_codigos.columns:
        mask_pausa = df_codigos['tipo'].astype(str).str.strip().str.upper().isin(['DESNCONSIDERAR', 'DESCONSIDERAR'])
    ...
    for maq in todas_maquinas:
        ...
        if info and info.get('status') == 'Parado':
            ...
            # Conta perda ativa APENAS se for problema (não for pausa programada)
            if not info['is_pausa']:
                try:
    ...
    if not df_hoje.empty:
        for _, row in df_hoje.iterrows():
            ...
            # Conta perda fechada APENAS se não for pausa programada
        ...
        # Filtra as pausas para descobrir o Top Ofensor real
        df_problemas = df_hoje[~df_hoje['cod_ocorrencia'].astype(str).str.strip().isin(codigos_pausa)]
    ...
    for p in maquinas_paradas:
        if p.get('is_pausa'):
            noticias.append(f"☕ {p['maquina']} em intervalo: {p['descricao_completa']}")
        else:
    ...
    c3.markdown(f"<div style='background:#fdedec; padding:15px; border-radius:10px; text-align:center; border:1px solid #f5b7b1;'><h4 style='margin:0; color:#c0392b;'>Paradas/Pausas</h4><h2 style='margin:0; font-size:38px; color:#e74c3c;'>{qtd_paradas} <span style='font-size:18px;'>({perc_paradas:.0f}%)</span></h2></div>", unsafe_allow_html=True)

    m1, m2, m3 = st.columns(3)
    m1.markdown(f"<div style='background:#fff; padding:15px; border-radius:10px; text-align:center; box-shadow: 0 2px 5px rgba(0,0,0,0.05); margin-top: 10px;'><h5 style='margin:0; color:#7f8c8d; text-transform: uppercase;'>🩸 Tempo Útil Perdido Hoje</h5><h3 style='margin:0; font-size:28px; color:#c0392b;'>{h_perdido:02d}h:{m_perdido:02d}m</h3></div>", unsafe_allow_html=True)
    ...
    # ==========================================
    st.markdown("<h3 style='text-align: center; color: #c0392b; text-transform: uppercase; font-weight: 900; margin-bottom: 20px;'>🚨 Painel de Status (Paradas e Pausas)</h3>", unsafe_allow_html=True)
    ...
    if not maquinas_paradas:
        ...
        <style>
            ...
            .card-pausa .alerta-icone { display: inline-block; }
        </style>
        """, unsafe_allow_html=True)
    ...
    # ==========================================
    # HISTÓRICO INDIVIDUAL POR MÁQUINA (GRÁFICO DE FITA)
    # ==========================================
    ...
    # 0=Fora Turno (Cinza), 1=Produção (Verde), 2=Parada (Vermelho), 3=Futuro (Branco), 4=Pausa/Lanche (Amarelo)
    color_map = {0: "#95a5a6", 1: "#27ae60", 2: "#e74c3c", 3: "#ecf0f1", 4: "#f39c12"}
    ...
    for setor in sorted(setores_dict.keys()):
        ...
        for m in range(total_timeline_min):
            ...
            if dist_inicio_m < 15 or dist_fim_m < 15 or dist_inicio_t < 15 or dist_fim_t < 15:
                continue
        ...
        for maq in sorted(setores_dict[setor]):
            ...
            # 1. Preenchimento Base (Verde / Cinza / Branco) - Sem pintura automática de lanche!
            ...
            # 2. Sobreposição APONTAMENTOS FECHADOS (Vermelho ou Amarelo)
            ...
            for _, row in maq_stops.iterrows():
                ...
                # Se for código de desconsiderar, pinta de amarelo(4), senão vermelho(2)
                cor_linha = 4 if c_oco in codigos_pausa else 2
                
                for m in range(inicio, fim):
                    idx = m - m_das_min
                    if 0 <= idx < total_timeline_min:
                        timeline[idx] = cor_linha
                        
            # 3. Sobreposição APONTAMENTOS ATIVOS AGORA
            for p in maquinas_paradas:
                if p['maquina'] == maq:
                    try:
                        ...
                        if h_ini_obj.date() == agora.date():
                            ...
                            fim = agora_min + 1 
                            c_oco = str(p.get('cod_ocorrencia')).strip()
    ...
    html_timelines += "<div style='display: flex; align-items: center; gap: 6px;'><div style='width:14px; height:14px; background:#f39c12; border-radius:3px;'></div> Pausa Programada</div>"
                ...
                paradas.forEach(p => {{
                    ...
                    if (distance > 0) {{
                        ...
                        if (cardEl && distance >= tempoCriticoMs) {{
                            // Garante que o alerta crítico (tela piscando) NÃO dispare para pausas amarelas!
                            if (!cardEl.classList.contains("card-critico") && !cardEl.classList.contains("card-pausa")) {{
